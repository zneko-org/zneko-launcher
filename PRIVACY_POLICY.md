...
If you choose to receive service credentials from ZNeko Link (such as ScreenScraper, SteamGridDB, RetroAchievements, or your S3 storage connection string), the transfer is initiated only on your request and requires explicit confirmation in ZNeko Link on your companion device. Credentials travel directly over the encrypted local TLS connection, are saved locally in your selected ZNeko data folder, and are never sent to or routed through ZNeko servers.
